## HFT Extra Coupon Scraper (hft-scrape-extra.py)
# by Kevin 'pnd4' Tran
...
def cFilter(cString):
    words = ["Small", "Medium", "Large", "X-Large" "EPA/CARB"]
    for word in words:
...
# Find all coupons
cpns = []
links = s.select('a[href*="ccdenc"]')
for cp in links:
    cp.img.decompose()
    if cp.get("target") == "_blank":
        cpns.append(cp)
